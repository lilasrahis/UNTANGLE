...
def sample_neg(net, train_pos=None, test_neg=None,test_pos=None, max_train_num=None):
...
def links2subgraphs(A, train_pos, train_neg, test_pos, test_neg, h=1,
    ...
    min_n_label = {'value': 1000} #EDIT: NODE
    max_n_label = {'value': 0}
    def helper(A, links, g_label):
        g_list = []
        if no_parallel:
            for i, j in tqdm(zip(links[0], links[1])):
                print ("Checking nodes i and j: "+str(i)+" "+str(j))
